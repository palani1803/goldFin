...
export interface NavbarProps {
  currentPage?: 'home' | 'live-rate' | 'gold-loan' | 'about'
  onNavigateHome?: () => void
...
export default function Navbar({
  ...
  const handleNavClick = (action: 'home' | 'about' | 'live-rate' | 'gold-loan' | string) => {
  ...
  return (
    <header className="sticky top-0 z-50 w-full bg-[#121212]/85 backdrop-blur-xl border-b border-white/10">
      <div className="max-w-[1320px] mx-auto px-4 md:px-6 flex items-center justify-between h-[78px]">
        ...
        >
          <div className="w-11 h-11 rounded-2xl bg-gradient-to-br from-[#F3C55B] via-[#DAAE4D] to-[#C89B2A] flex items-center justify-center text-[#121212] shadow-[0_6px_30px_rgba(234,179,8,0.35)] group-hover:scale-105 transition-transform shrink-0">
            <Coins size={24} />
          </div>
        ...
        <nav className="hidden md:flex items-center gap-8 ml-auto">
          ...
          {/* 4. About Us Tab (Positioned after Gold Loan) */}
          <button
      ...
      {mobileMenuOpen && (
        <div className="md:hidden bg-[#161616] border-b border-white/10 px-6 py-5 flex flex-col gap-4 animate-in slide-in-from-top-2">
          <button
            onClick={() => handleNavClick('home')}
